...
import { UploadCloudIcon, CheckCircleIcon, EditIcon, SearchIcon, SparklesIcon } from './icons';
...
interface WardrobePanelProps {
  ...
  onOpenGenerateGarment: () => void;
}

const WardrobePanel: React.FC<WardrobePanelProps> = ({ onGarmentSelect, activeGarmentIds, isLoading, wardrobe, onOpenEditGarment, onOpenGenerateGarment }) => {
  ...
  return (
    <div className="pt-6 border-t border-gray-400/50">
        ...
        <div className="grid grid-cols-3 gap-3">
            ...
            <button 
                onClick={onOpenGenerateGarment}
                disabled={isLoading}
                className="relative aspect-square border-2 border-dashed rounded-lg flex flex-col items-center justify-center text-gray-500 transition-colors disabled:cursor-not-allowed disabled:bg-gray-100 hover:border-gray-400 hover:text-gray-600 cursor-pointer"
                aria-label="Generate garment with AI"
            >
                <SparklesIcon className="w-6 h-6 mb-1"/>
                <span className="text-xs text-center">Generate</span>
            </button>
        </div>
